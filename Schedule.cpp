...
{
    map<string,vector<Action*> > t2op = operationsByThread;
    ...
    if(!bugFixMode)
    {
        for(int i = 0; i < globalOrder.size(); i++)
        {
            ...
            string tid = "Not Implement";//operationLIB::parseThreadId(op);
...
std::vector<std::string> scheduleLIB::schedule2string(const Schedule& schedule)
{
    ...
    for(Schedule::const_iterator it = schedule.begin() ; it != schedule.end(); it ++)
        listOp.push_back((*it)->get_action_str());
    return listOp;
}
...
string scheduleLIB::getTidOperation(Action op)
{
    return op.get_tid();
}
...
vector<string> scheduleLIB::getSolutionStr(Schedule schedule){
    ...
    {
        actionsList.push_back((*it)->get_action_str());
        i++;
    }
    return actionsList;
}
...
void scheduleLIB::fillScheduleOrd(string tid, map<string,vector<Action*> >* op_list, Schedule* sch)
{
    ...
    //remove the head
    vector<Action*>::iterator it_erase = (*op_list)[tid].begin();
